# SWFL Food Finder (Prototype)

Mobile-first prototype for a personalized Southwest Florida restaurant recommendation app.

## Included pages / sections
- Login / signup
- Onboarding preference setup
- Personalized home recommendations
- Discovery list with filters
- Individual restaurant detail view
- Favorites and want-to-try lists
- Notifications trigger
- Profile / preference summary

## Run locally (preview)
1. Start a local static server from this repo:
   ```bash
   python3 -m http.server 4173
   ```
2. Open your browser to:
   - `http://localhost:4173`
   - or `http://127.0.0.1:4173`

## Interactivity
All primary buttons now trigger visible in-app feedback via the status line under the header, including:
- email login submit
- Google / Apple demo buttons
- notification button
- recommendation refresh
- filter and preference controls
- restaurant card actions (details, favorite, want to try)
- bottom navigation buttons

## Personalization logic
Recommendations are scored by:
- preferred cuisine match
- budget match
- vibe/context match
- dietary fit
- local/independent weighting
- base rating
