  ...
  article.innerHTML = `
    ...
    <div class="copy">
      ...
      <div class="row">
        ...
        <button class="ghost" data-action="favorite">${state.favorites.has(restaurant.id) ? "Favorited" : "Favorite"}</button>
        <button class="ghost" data-action="want">${state.wantToTry.has(restaurant.id) ? "Saved" : "Want to try"}</button>
      </div>
    </div>
  `;
  article.querySelector('[data-action="details"]').onclick = () => showDetails(restaurant);
...
function buildUI() {
  ...
  byId("profileSummary").textContent = `Cuisine: ${state.selectedCuisine.join(", ")} · Budget: ${state.budget} · Diet: ${state.selectedDiet.join(", ")} · Context: ${state.selectedVibes.join(", ")}`;
...
function setupEvents() {
  ...
  byId("refreshRecs").onclick = () => buildUI();

  byId("notifyBtn").onclick = () => {
    byId("notificationCount").textContent = "0";
    alert("No new notifications. Trending now: waterfront seafood in Naples.");
  };
